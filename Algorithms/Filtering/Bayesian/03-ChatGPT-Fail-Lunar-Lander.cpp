...
void bayesianFilter(float state[], float measurement[], float control[], float dt, float target_rate) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // draw the LEM
        int lem_y = HEIGHT - (int)(state[i] * PIXEL_TO_METER) - LEM_SIZE;
    ...
    // draw the thrust
    if(throttle > 0.6) {
        cv::line(image, cv::Point(WIDTH/2, lem_y + LEM_SIZE), cv::Point(WIDTH/2, HEIGHT - MOON_HEIGHT), cv::Scalar(0, 0, 255), 5);
    }
    // display the image
