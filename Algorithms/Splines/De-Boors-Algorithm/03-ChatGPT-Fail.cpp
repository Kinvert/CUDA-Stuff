...
std::vector<std::vector<std::vector<double>>> evaluateBSpline(const std::vector<double>& knots, const std::vector<std::vector<double>>& controlPoints, int degree, double t)
{
    ...
    std::vector<std::vector<std::vector<double>>> d(numControlPoints);
    for (int i = 0; i < numControlPoints; i++)
    {
        d[i].resize(degree + 1);
        for (int j = 0; j <= degree; j++)
        {
            d[i][j].resize(controlPoints[0].size());
        }
    }
