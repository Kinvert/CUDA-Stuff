...
int main()
{
    // allocate memory on the device and copy the host balls to the device
    cudaMalloc((void**)&deviceBalls, numBalls * sizeof(Ball));
    cudaMemcpy(deviceBalls, hostBalls.data(), numBalls * sizeof(Ball), cudaMemcpyHostToDevice);

    // initialize the balls with random positions
    for (int i = 0; i < numBalls; i++)
    {
        Ball& ball = hostBalls[i];
        ball.x = dist(rng);
    }
