...
int main(int argc, char** argv) {
    ...
    {
        int channelCount;
        imageData = stbi_load("image.jpg", &width, &height, &channelCount, CHANNEL_COUNT);
        if (imageData == NULL) {
    ...
    // Save output image
    stbi_write_png("output.png", width, height, CHANNEL_COUNT, outputData, width * CHANNEL_COUNT);
